...
int RGWHandler_ObjStore_SWIFT::init_from_header(struct req_state *s)
{
  ...
  /*
   * While "tenant\bucket" syntax works fine in S3, this method presents
   * a compatibility issue in Swift if someone created a container with
   * backslash in its name. We ought to change our auth so it gives away
   * tenants in StorageURL: headers, then pick tenant name from URL.
   */
...
int RGWHandler_ObjStore_SWIFT::init(RGWRados *store, struct req_state *s, RGWClientIO *cio)
{
  ...
  if (copy_source) {
    bool result = RGWCopyObj::parse_copy_location(copy_source,
        s->src_tenant_name, s->src_bucket_name, s->src_object);
    if (!result)
       return -ERR_BAD_URL;
  }
  ...
  if (s->op == OP_COPY) {
    ...
    bool result = RGWCopyObj::parse_copy_location(req_dest,
        dest_tenant_name, dest_bucket_name, dest_obj_key);
    if (!result)
       return -ERR_BAD_URL;
